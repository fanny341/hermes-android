...
class ApiClient {
  ...
  // ── Native API server endpoints (port 8642, no dashboard needed) ──

  /// GET /v1/toolsets — list all toolsets with enabled/configured status.
  /// Works without the dashboard. Shows what tools the agent can use.
  Future<List<Map<String, dynamic>>> getToolsets() async {
    final res = await _http.get(
      Uri.parse('$baseUrl/v1/toolsets'),
      headers: _headers,
  ...
  /// GET /v1/skills — list skills via the API server (no dashboard required).
  /// Returns name, description, category.
  Future<List<Map<String, dynamic>>> getApiSkills() async {
    final res = await _http.get(
  ...
  /// GET /v1/capabilities — machine-readable API feature list.
  Future<Map<String, dynamic>> getCapabilities() async {
    final res = await _http.get(
      Uri.parse('$baseUrl/v1/capabilities'),
      headers: _headers,
    ).timeout(const Duration(seconds: 10));
    if (res.statusCode != 200) throw Exception('HTTP ${res.statusCode}');
    return jsonDecode(res.body) as Map<String, dynamic>;
  ...
  /// Pass an empty body; server creates a new session from the lineage.
  Future<Map<String, dynamic>> forkSession(String sessionId) async {
    final encodedId = Uri.encodeComponent(sessionId);
    final res = await _http.post(
      Uri.parse('$baseUrl/api/sessions/$encodedId/fork'),
      headers: _headers,
      body: jsonEncode({}),
    ).timeout(const Duration(seconds: 15));
    if (res.statusCode < 200 || res.statusCode >= 300) {
    ...
    // Response shape: {"object":"hermes.session","session":{"id":...}}
    // Flatten so callers get the session fields directly.
    final nested = decoded['session'];
    if (nested is Map<String, dynamic>) {
      return {...nested, ...decoded..remove('session')};
    }
    return decoded;
  }

  /// POST /v1/runs — submit an async agent run.
  /// Returns immediately with a run_id; poll via getRunStatus().
  Future<Map<String, dynamic>> submitRun({
  ...
  Future<Map<String, dynamic>> getRunStatus(String runId) async {
    final res = await _http.get(
      Uri.parse('$baseUrl/v1/runs/${Uri.encodeComponent(runId)}'),
      headers: _headers,
    ).timeout(const Duration(seconds: 10));
    if (res.statusCode != 200) throw Exception('HTTP ${res.statusCode}');
    return jsonDecode(res.body) as Map<String, dynamic>;
  ...
  Future<void> stopRun(String runId) async {
    final res = await _http.post(
      Uri.parse('$baseUrl/v1/runs/${Uri.encodeComponent(runId)}/stop'),
      headers: _headers,
    ).timeout(const Duration(seconds: 10));
    if (res.statusCode < 200 || res.statusCode >= 300) {
      throw Exception('HTTP ${res.statusCode}');
